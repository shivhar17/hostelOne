...
export const Dashboard: React.FC = () => {
  ...
  const [refreshKey, setRefreshKey] = useState(0); // To force re-render images
  ...
  const handleTouchEnd = () => {
    ...
    if (pullY > 50) {
      ...
      setPullY(50); // Keep indicator visible
      performRefresh();
    } else {
  ...
  const performRefresh = () => {
    setTimeout(() => {
      ...
      setPullY(0);
      setRefreshKey(prev => prev + 1); // Simulate new data
    }, 1500);
  };
  ...
  return (
    ...
    >
      ...
      <div className="pt-12 px-6">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex items-center gap-2">
             <div className="bg-cyan-500 p-2 rounded-xl text-white shadow-md shadow-cyan-200 dark:shadow-none relative">
                ...
                <div className="absolute -top-1.5 -right-1.5 bg-white text-cyan-500 text-[10px] font-extrabold w-5 h-5 flex items-center justify-center rounded-full border-2 border-cyan-500">1</div>
             </div>
             <span className="text-2xl font-bold text-orange-500 tracking-tight">HostelOne</span>
          </div>
          ...
          <div className="flex items-center gap-3">
             <button 
              ...
              <div onClick={() => navigate('/profile')} className="bg-orange-100 dark:bg-orange-900/40 p-1 rounded-full transition-colors cursor-pointer active:scale-95">
                <img 
        ...
        <h1 className="text-3xl font-bold text-slate-800 dark:text-white mb-8 transition-colors">Good Morning, {getFirstName(userProfile.name)}!</h1>

        {/* Menu Card */}
        <motion.div 
          key={refreshKey} // Re-animate on refresh
        ...
        >
          ...
          <div className="relative z-10">
            <div className="flex justify-between items-start mb-4">
               <div className="bg-emerald-50 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-xs font-bold px-3 py-1.5 rounded-full uppercase tracking-wide">Today's Menu</div>
               <img src={`https://picsum.photos/150/150?random=food${refreshKey}`} className="w-20 h-20 object-cover rounded-full shadow-md border-4 border-white dark:border-slate-700" alt="Food" />
            </div>
            
            <div className="space-y-3">
               <div>
                  <span className="text-xs text-slate-400 dark:text-slate-500 font-semibold uppercase block mb-1">Breakfast</span>
                  <p className="text-slate-700 dark:text-slate-200 font-medium">Poha, Tea, Toast</p>
               </div>
               <div>
                  <span className="text-xs text-slate-400 dark:text-slate-500 font-semibold uppercase block mb-1">Lunch</span>
                  <p className="text-slate-700 dark:text-slate-200 font-medium">Roti, Dal Fry, Jeera Rice, Salad</p>
               </div>
            </div>
...
const MenuButton: React.FC<MenuButtonProps> = ({ icon: Icon, label, subLabel, color, iconColor, onClick }) => (
  <motion.button 
    whileTap={{ scale: 0.98 }}
    onClick={onClick}
  ...
  >
    ...
    <div>
      <h3 className="text-slate-800 dark:text-slate-100 font-bold text-lg leading-tight mb-1">{label}</h3>
      <p className="text-slate-500 dark:text-slate-400 text-xs font-medium">{subLabel}</p>
    </div>
  </motion.button>
);